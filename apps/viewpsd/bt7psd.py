...
To read psd and plot with matplotlib.pylab.hexbin
@author: yzhao
"""
...
def psdcali(al2o3file,calipath = 'psdcali/',a4c = None, vfile = None,veff = zeros(48),Ef = 0,pl = None, califile = None,psdrange = range(48),labels = '$Al_2O_3$', BGeff = True):
    if califile == None:
        califile = calipath + 'psdCali_%s.dat' %time.strftime("%b%d_%Y", time.localtime())
    psdcol = ['Psdc%02d' %n for n in range(48)]
    al2da = bt7read(al2o3file,col = ['A4']+psdcol)
    efl = zeros(48)+Ef
    ...
    def fita4():
        ...
        for i in psdrange:
            ...
            if pl:
                subplot(3,4,i/4+1)
                errorbar(xx,yy,sqrt(yy),fmt = '%so' %clist[i%4],label = 'PSD%02d' %i)
...
#        print intt,intterr
    ...
    if pl:
        ...
        savefig(calipath + 'PSDeff_'+pl)
    califd = open(califile,'w')
    califd.write('#BT7 PSD calibration based on %s\n' %al2o3file)
...
def psdcalidave(al2o3file,a4c = None, vfile = None,veff = zeros(48),pl = None, Ef = 14.7,
    ...
    # old style which is not used in new PSD Dave package.
...
def readpsd(files,psdind = range(48),Ei = 14.7, Efc = 14.7,skip = True,
            califile = 'psd_cali.dat',savedata = None):
    psdeff1,a4d, Ef = loadtxt(califile,unpack = True)
    ki = sqrt(Ei/2.072)
    ...
    for fn in files:
        data = bt7read(fn,col = ['A3','A4','Monitor','Time']+['Psdc%02d' %n for n in range(48)])
        a3,a4,imon,itime = data[:,0],data[:,1],data[:,2],data[:,3]
        for i in psdind:
            qq, qxx,qyy = convertq(ki,kf[i],a3,a4-a4d[i])
            ccc = data[:,i+4]/psdeff1[i]
            qx = r_[qx,qxx]
            qy = r_[qy,qyy]
            cc = r_[cc,ccc]
            mon = r_[mon,imon]
...
def plotpsdmap(files,califile = 'psd_cali.txt', Ei = 14.7, gridl = None, bins = None,
               psdind = range(0,48),baseMon = None, xrec = 1, yrec = 1, extent = None,gridsize = 200,
    ...
    plr= 0.8
    cbw = 0.4
    xx,yy,cc,mon,time = readpsd(files,psdind = psdind, califile = califile, savedata = savedata, Ei = Ei, skip = skip)
...
#    fd = open('qpsd.dat','w')
    a4d = loadtxt('PSD_a4space.dat')-39.59
    da = bt7read('./data/QscanPSD91189.bt7',['A3','A4','Ei','Ef'])
...
def psdpowder(fn,psdind = range(48),Ei = 14.7, califile = 'psd_cali.dat',savedata = None, bins = True, bina4 = 0.25, appeff = True):
    psdeff1,a4d, Ef = loadtxt(califile,unpack = True)
    ki = sqrt(Ei/2.072)
#    kf = sqrt((Ef+Efc)/2.072)
#    qx = empty(0)
